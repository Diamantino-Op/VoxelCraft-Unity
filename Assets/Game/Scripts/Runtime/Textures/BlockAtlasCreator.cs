...
namespace VoxelCraft.Textures
{
    ...
    {
        public static Atlas generateAlbedoAtlas(Dictionary<int, Block> blocks)
        {
            int blockSize = 16;
            ...
            {
                foreach(Texture2D texture in block.Value.albedoTextures)
                {
            ...
            Texture2D atlas = new Texture2D(atlasSize, atlasSize, TextureFormat.ARGB32, false);
        ...
        public static Texture2D generateNormalAtlas(Dictionary<int, Block> blocks)
        {
            int blockSize = 16;
            int atlasSizeInBlocks = (int)Mathf.Sqrt(blocks.Count);
            ...
            foreach (KeyValuePair<int, Block> block in blocks)
            {
                foreach (Texture2D texture in block.Value.normalTextures)
                {
                    if (!uvs.ContainsKey(texture.name))
                    {
                        if (texture.width == blockSize && texture.height == blockSize)
